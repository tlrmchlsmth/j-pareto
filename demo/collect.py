#!/usr/bin/env python3
"""Collect demo sweep results from Kubernetes Job logs.

Reads structured sweep output from kubectl logs of demo Job pods
...
Usage:
    python3 demo/collect.py -n agg-tp2-12r -n disagg-8p4d-kv
    python3 demo/collect.py -n agg-tp2-12r -n disagg-8p4d-kv --sheets "Demo Results"
"""
...
def parse_sweep_log(log_text: str) -> tuple[dict[str, str], list[BenchResult]]:
    """Parse a sweep log (same format as job.yaml structured output).
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect demo sweep results from Kubernetes Job logs"
    )
    parser.add_argument(
